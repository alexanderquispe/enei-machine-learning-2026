...
Two main options for package managers: \texttt{pip} and \texttt{conda}
\subsubsection{pip}
...
\subsubsection{conda}
\begin{lstlisting}[language=bash]
conda -n venv numpy pandas scikit-learn matplotlib jupyter ipykernel -y && conda activate venv
\end{lstlisting}
...
form $\vX\T\vX$ and $\vX\T\vy$, and solve. You must obtain $\hat\vbeta=(1.49,\,0.75)\T$ --- Lecture 1, slide 16.
\end{task}
...
Explain, in one sentence each, which term of the bias--variance decomposition (Lecture 1, slide 42) is
...
loss? Which one gets close fastest in wall-clock time? Explain both with Lecture 1, slides 37--38.
\end{exercise}
\begin{exercise}
...
\section*{Submission}
A single notebook in Colab, all cells executed top to bottom without error (\emph{Kernel $\to$ Restart \& Run All}). 
The share link for the Colab notebook must be shared through the platform before Wednesday, September 16, 19:00. Name it \texttt{lab01\_[lastname]\_[firstname].ipynb}.
